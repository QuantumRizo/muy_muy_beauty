...
\begin{titlepage}
  ...
  {\color{white}\Huge\bfseries MUYMUY Beauty Studio}\\[0.5cm]
  {\color{primario}\Large Manual de Usuario}\\[0.3cm]
  {\color{white}\LARGE\bfseries ROL: ADMINISTRADOR}\\[1cm]
  {\color{primario}\rule{\linewidth}{1pt}}\\[2cm]
  {\color{gris}\large Versión 1.0 --- \today}\\[0.5cm]
  {\color{gris}\normalsize Sistema Administrativo Interno}
  \vfill
  {\color{white}\small Documento confidencial --- Solo para uso interno}
\end{titlepage}
\nopagecolor
...
\begin{adminbox}
El administrador tiene acceso a \textbf{todas} las secciones: Inicio, Agenda, Clientes, Asistencia, Venta Directa, Caja, \textbf{Análisis, Administración, Inventario} y \textbf{Marketing}.
\end{adminbox}
...
La sección de Marketing permite gestionar campañas de comunicación con los clientes.

\subsection{Campañas de WhatsApp}
...
\subsection{Configuración de Mensajes Automáticos}

Puedes configurar mensajes automáticos que el sistema envía sin intervención manual:
\begin{itemize}[leftmargin=*]
  \item \textbf{Confirmación de cita:} Se envía cuando se agenda una nueva cita.
  \item \textbf{Recordatorio 24h:} Se envía el día anterior a la cita.
  \item \textbf{Felicitación de cumpleaños:} Se envía el día del cumpleaños del cliente.
\end{itemize}
...
\begin{center}
  ...
  \textcolor{gris}{\small MUYMUY Beauty Studio --- Manual de Administrador v1.0}\\
